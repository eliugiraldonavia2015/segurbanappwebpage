...
export const CapabilitiesSection: React.FC<CapabilitiesSectionProps> = ({ onOpenDemo }) => {
  ...
  return (
    <section id="benefits" className="min-h-screen py-16 bg-slate-900 relative flex flex-col justify-center">
      {/* Background */}
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 max-w-2xl mx-auto">
            Diseñamos interfaces específicas para cada rol, garantizando eficiencia y control total en cada interacción.
          </p>
        </div>
        ...
        <div className="bg-slate-800/30 backdrop-blur-md border border-slate-700/50 rounded-3xl p-6 md:p-12 min-h-[500px]">
          <AnimatePresence mode="wait">
            <motion.div
              key={activeTab}
            ...
            >
              ...
              <div className="order-2 md:order-1">
                <div className={`inline-block px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider mb-4 bg-${content[activeTab].color}-500/20 text-${content[activeTab].color}-400 border border-${content[activeTab].color}-500/30`}>
                   Beneficios Clave
                </div>
                ...
                <div className="grid gap-4">
                  {content[activeTab].features.map((feature, idx) => (
                    ...
                    >
                      <div className={`p-2 rounded-lg bg-${content[activeTab].color}-500/10 text-${content[activeTab].color}-400 shrink-0`}>
                ...
                <div className="mt-10">
                   <button 
                      onClick={onOpenDemo}
                      className={`bg-${content[activeTab].color}-600 hover:bg-${content[activeTab].color}-500 text-white font-bold py-3 px-8 rounded-xl shadow-lg transition-all flex items-center gap-2`}
                   >
                      Ver Demo para {tabs.find(t => t.id === activeTab)?.label.replace('Para ', '')} <ArrowRight size={18} />
                   </button>
                </div>
              </div>
              ...
              <div className="order-1 md:order-2 h-[300px] md:h-[500px] flex items-center justify-center relative">
                 ...
                 <div className={`absolute inset-0 bg-${content[activeTab].color}-500/20 blur-[100px] rounded-full pointer-events-none`}></div>
